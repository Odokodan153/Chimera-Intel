...
def start_daemon():
    ...
    interval_seconds = active_project.daemon_config.monitoring_interval_hours * 3600
    target = active_project.domain
    workflow = active_project.daemon_config.workflow

    while True:
        logger.info("Daemon is running monitoring workflow.")
        _run_workflow(workflow, target)
        logger.info(f"Workflow complete. Sleeping for {interval_seconds} seconds.")
