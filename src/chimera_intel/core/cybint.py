...
async def generate_attack_surface_report(domain: str) -> AttackSurfaceReport:
    ...
    with console.status("[bold cyan]Gathering intelligence from multiple modules...[/bold cyan]"):
        footprint_task = gather_footprint_data(domain)
        # Vulnerability scanner is sync, so we run it in a thread
        vuln_scan_task = asyncio.to_thread(run_vulnerability_scan, domain, None) # API key is handled internally
    ...
    summary_data = {
        ...
        "footprint_summary": {
            ...
            "ip_threat_intel_hits": len(footprint_results.footprint.ip_threat_intelligence),
        },
    ...
    if google_api_key:
        with console.status("[bold cyan]Generating AI-powered risk summary...[/bold cyan]"):
            prompt = f"""
            ...
            ```
            """
            summary_result = generate_swot_from_data(prompt, google_api_key) # Re-using the generic AI function
...
async def run_attack_surface_analysis(
    ...
    output_file: str = typer.Option(None, "--output", "-o", help="Save the full report to a JSON file."),
):
    """
    ...
    console.print("\n" + "="*50)
    console.print(f"[bold green]Attack Surface Risk Assessment for: {domain}[/bold green]")
    console.print("="*50 + "\n")
    console.print(results_model.ai_risk_assessment)
    console.print("\n" + "-"*50)
    console.print("[dim]Full detailed results are available in the JSON output.[/dim]")

    if output_file:
        results_dict = results_model.model_dump(exclude_none=True)
        save_or_print_results(results_dict, output_file)
        save_scan_to_db(target=domain, module="cybint_attack_surface", data=results_dict)