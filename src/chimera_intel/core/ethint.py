...
def load_frameworks(path: str = "src/chimera_intel/core/ethical_frameworks.json") -> Dict:
    ...
    try:
        ...
        audit_logger.critical(f"FATAL: Could not decode JSON from ethical frameworks file: {path}")
        return {}
...
def audit_operation(operation: Operation, frameworks_to_check: List[str]) -> ComplianceResult:
    ...
    log_action(f"AUDIT_START - OpID: {operation.operation_id}, Type: {operation.operation_type}")
    ...
    try:
        ...
        log_action("CRITICAL - Could not import the rules module 'chimera_intel.core.ethint_rules'.")
    ...
    for framework_name in frameworks_to_check:
        ...
        log_action(f"Checking against framework '{framework_name}' version {framework_version}")
        ...
        for rule in framework.get("rules", []):
            ...
            if not check_function:
                log_action(f"WARNING - OpID: {operation.operation_id}, No check function found for rule: {rule_id} (expected: {check_function_name})")
            ...
            try:
                if not check_function(operation):
                    ...
                    log_action(f"VIOLATION - OpID: {operation.operation_id}, Rule: {rule_id}, Severity: {severity}")
            except Exception as e:
                log_action(f"ERROR - OpID: {operation.operation_id}, Rule: {rule_id} failed to execute: {e}")
    ...
    if unchecked_rules:
        ...
        log_action(f"NON_COMPLIANT - OpID: {operation.operation_id}, Missing rule implementations: {', '.join(unchecked_rules)}")
...
def run_audit(
    ...
    severity_level: str = typer.Option("LOW", "--severity-level", "-s", help="Minimum severity level to display (LOW, MEDIUM, HIGH, CRITICAL).")
):
    ...
    if min_severity is None:
        console.print(f"[bold red]Invalid severity level '{severity_level}'. Choose from LOW, MEDIUM, HIGH, CRITICAL.[/]")
    ...
    if result.is_compliant:
        ...
        if filtered_violations:
            table = Table(title=f"Compliance Violations (Severity >= {severity_level.upper()})")
            ...
            console.print(f"[yellow]No violations found at or above severity level '{severity_level.upper()}'.[/]")
