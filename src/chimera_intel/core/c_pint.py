...
from .ot_intel import OTAsset
from .geoint import GeoLocation
from .sigint import SignalIntercept
from .vulnerability_scanner import Vulnerability
from .schemas import CPSAnalysisResult, CascadingFailurePath
...
def analyze_cps_for_cascading_failures(graph: nx.Graph) -> Tuple[CPSAnalysisResult, nx.Graph]:
    """
        ...
        for i, start_node in enumerate(critical_nodes):
            for end_node in critical_nodes[i+1:]:
                if nx.has_path(graph, start_node, end_node):
                    for path in nx.all_simple_paths(graph, source=start_node, target=end_node, cutoff=5):
...
def run_cps_analysis(
    project_file: str = typer.Option(..., help="Path to a JSON project file with all intelligence data."),
):
    """
    ...
    try:
        ...
        signal_intercepts = [SignalIntercept(**s) for s in data.get("signal_intercepts", [])]
    ...
    with console.status("[bold green]Modeling and analyzing the cyber-physical system...[/]"):
        graph = model_cyber_physical_system(ot_assets, geo_locations, signal_intercepts, vulnerabilities)
