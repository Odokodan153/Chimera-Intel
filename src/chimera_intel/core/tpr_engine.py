...
async def run_full_tpr_scan(domain: str) -> TPRMReport:
    ...
    vuln_results, breach_results = await asyncio.gather(vuln_scan_task, breach_scan_task)
    ...
    if google_api_key:
        ...
        summary_result = generate_swot_from_data(prompt, google_api_key) # Misnomer, but it just runs a prompt
...
def run_tpr_scan_command(
    ...
    """
    with console.status(f"[bold cyan]Running comprehensive TPRM scan on {domain}... This may take a while.[/bold cyan]"):
