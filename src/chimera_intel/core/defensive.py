...
def find_typosquatting_dnstwist(domain: str) -> dict:
    ...
    Returns:
        dict: A dictionary of the dnstwist JSON output, or an error message.
    """
    try:
        # SECURE IMPLEMENTATION: Pass the command and arguments as a list.
        # This avoids using the shell (`shell=False` is the default) and prevents command injection.
        command = ["dnstwist", "--json", domain]
        
        process = subprocess.run(
            command,
            capture_output=True, text=True, check=True, timeout=120 # Increased timeout for long scans
        )
...
def search_pastebin_psbdmp(query: str) -> dict:
    ...
    Returns:
        dict: A dictionary of found pastes, or an error message.
    """
    try:
        # SECURE IMPLEMENTATION: Pass the command and arguments as a list.
        ...
        # psbdmp often exits with an error code when no results are found.
...
def analyze_ssl_ssllabs(host: str) -> dict:
    ...
    Returns:
        dict: A dictionary containing the full SSL Labs report, or an error message.
    """
    ...
    def start_scan(hostname):
        ...
        response = requests.get(api_url + "analyze", params=payload, timeout=30)
        response.raise_for_status()
...
def analyze_apk_mobsf(file_path: str, mobsf_url: str, api_key: str) -> dict:
    ...
    Returns:
        dict: The full JSON report from MobSF, or an error message.
    """
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            upload_response = requests.post(f"{mobsf_url}/api/v1/upload", headers=headers, files=files, timeout=120)
            upload_response.raise_for_status()
        upload_data = upload_response.json()
        
        console.print(" [cyan]>[/cyan] Starting MobSF scan...")
        scan_response = requests.post(f"{mobsf_url}/api/v1/scan", headers=headers, data=upload_data, timeout=120)
        scan_response.raise_for_status()
        
        console.print(" [cyan]>[/cyan] Fetching MobSF JSON report...")
        report_response = requests.post(f"{mobsf_url}/api/v1/report_json", headers=headers, data=upload_data, timeout=120)
...
# Defines the 'defensive' command group and all its sub-commands.
