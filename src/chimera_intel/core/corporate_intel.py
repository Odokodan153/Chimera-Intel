...
def run_hr_intel(
    ...
    if not target_name:
        ...
        if active_project:
            # Use domain for hiring trends, company_name for sentiment
    ...
    if not target_name:
        ...
        raise typer.Exit(code=1)
    # Assume domain if it contains a '.', otherwise assume company name for sentiment

    is_domain = "." in target_name and " " not in target_name

    hiring_results = (
        get_hiring_trends(target_name)
        if is_domain
        else HiringTrendsResult(
            total_postings=0, error="Domain needed for hiring trends."
        )
    )
    sentiment_results = get_employee_sentiment(target_name)
