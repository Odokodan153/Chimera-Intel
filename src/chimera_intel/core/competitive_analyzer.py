...
def run_competitive_analysis(
    ...
    with console.status(
        f"[bold cyan]Fetching historical data for both targets...[/bold cyan]"
    ):
    ...
    if not target_a_data or not target_b_data:
        console.print(
            f"[bold red]Error:[/bold red] Could not retrieve historical data for one or both targets. Ensure scans have been run for both."
        )
