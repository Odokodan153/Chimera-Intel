...
@patch(
    "chimera_intel.core.autonomous.requests.post",
    side_effect=requests.exceptions.RequestException("Test Request Error"),
)
@patch("builtins.open", new_callable=mock_open)
def test_trigger_retraining_pipeline_request_exception(
    mock_file_open, mock_post, mock_api_keys, capsys
):
    ...
    assert (
        "Optimization plan saved to model_optimization_plan_failed.txt" in captured.out
    )
...
def test_optimize_models_auto_trigger(
    mock_trigger, runner, mock_db_connection, mock_api_keys, mock_ai_task
):
...
def test_optimize_models_ai_error(
    runner, mock_db_connection, mock_api_keys, mock_ai_task_error
):
...
def test_detect_drift_detected(mock_ks_2samp, mock_read_csv, runner):
    ...
    result = runner.invoke(
        autonomous_app, ["detect-drift"], input="baseline.csv\nnew.csv\n"
    )
...
@patch("chimera_intel.core.autonomous.ks_2samp")
@patch("chimera_intel.core.autonomous.trigger_retraining_pipeline")
def test_detect_drift_auto_trigger(
    mock_trigger, mock_ks_2samp, mock_read_csv, runner, mock_api_keys
):
...
@patch(
    "chimera_intel.core.autonomous.pd.read_csv",
    side_effect=FileNotFoundError("File not found: baseline.csv"),
)
...
@patch(
    "chimera_intel.core.autonomous.pd.read_csv",
    side_effect=Exception("General CSV error"),
)
