...
class TestBusinessIntel(unittest.TestCase):
    ...
    async def test_get_news_gnews_success(self, mock_get: AsyncMock):
        """
        Tests a successful news retrieval from the GNews API.

        Args:
            mock_get (AsyncMock): A mock for the `httpx.AsyncClient.get` method.
        """
        ...
        result = await get_news_gnews("Apple Inc.", "fake_api_key")
    ...
    async def test_get_news_gnews_no_api_key(self):
        ...
        result = await get_news_gnews("Test Query", "")
        self.assertIsInstance(result, GNewsResult)
        self.assertIsNotNone(result.error)
        self.assertIn("API key not found", result.error)

    @patch("chimera_intel.core.http_client.async_client.get", new_callable=AsyncMock)
    async def test_get_news_gnews_http_error(self, mock_get: AsyncMock):
        """
        ...
        result = await get_news_gnews("Test Query", "fake_api_key")
        self.assertIsInstance(result, GNewsResult)
        self.assertIsNotNone(result.error)
        self.assertIn("503", result.error)

    @patch("chimera_intel.core.http_client.async_client.get", new_callable=AsyncMock)
    async def test_scrape_google_patents_success(self, mock_get: AsyncMock):
        """
        ...
        result = await scrape_google_patents("Apple Inc.")
    ...
    @patch("chimera_intel.core.http_client.async_client.get", new_callable=AsyncMock)
    async def test_scrape_google_patents_no_results(self, mock_get: AsyncMock):
        """
        ...
        result = await scrape_google_patents("A company with no patents")
        self.assertIsInstance(result, PatentResult)
        self.assertIsNotNone(result.patents)
        self.assertEqual(len(result.patents), 0)

    @patch("chimera_intel.core.http_client.async_client.get", new_callable=AsyncMock)
    async def test_scrape_google_patents_network_error(self, mock_get: AsyncMock):
        """
        ...
        result = await scrape_google_patents("Test company")
        self.assertIsInstance(result, PatentResult)
        self.assertIsNotNone(result.error)
    ...
    @patch("chimera_intel.core.business_intel.get_news_gnews", new_callable=AsyncMock)
    @patch(
        "chimera_intel.core.business_intel.scrape_google_patents",
        new_callable=AsyncMock,
    )
    @patch("chimera_intel.core.config_loader.API_KEYS.gnews_api_key", "fake_key")
    def test_cli_business_intel_with_ticker(
        self, mock_patents: AsyncMock, mock_news: AsyncMock, mock_financials: MagicMock
    ):
        """
        Tests the 'business' command when a ticker is provided.

        Args:
            mock_patents (AsyncMock): A mock for `scrape_google_patents`.
            mock_news (AsyncMock): A mock for `get_news_gnews`.
