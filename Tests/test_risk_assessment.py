...
def mock_vulnerabilities():
    ...
    return [
        ...
        Vulnerability(
            cve="CVE-2023-1003",
            cvss_score=5.0,
            description="Med vuln",
            severity="medium",
        ),
        # FIX: Changed severity to 'low' for a more realistic boost calculation
        Vulnerability(
            cve="CVE-2023-1004", cvss_score=2.0, description="Low vuln", severity="low"
        ),
    ]
...
def mock_threat_intel():
    ...
    return ThreatIntelResult(
        ...
        pulses=[
            PulseInfo(
                name="Pulse 1", tags=["ransomware"], malware_families=["lockbit"]
            ),
...
def test_calculate_risk_low():
    ...
    result = calculate_risk(
        asset="Server 1", threat="Data Theft", probability=0.1, impact=1.0
    )
...
def test_calculate_risk_medium():
    ...
    result = calculate_risk(
        asset="Server 1", threat="Data Theft", probability=0.5, impact=5.0
    )
...
def test_calculate_risk_high(
    runner,
):  # Added runner fixture, though unused, to match others
    """Tests a high risk calculation."""
    result = calculate_risk(
        asset="Server 1", threat="Data Theft", probability=0.8, impact=8.0
    )
...
def test_calculate_risk_critical(
    ...
    result = calculate_risk(
        asset="Server 1", threat="Data Theft", probability=0.9, impact=9.0
    )
...
@patch("chimera_intel.core.risk_assessment.search_threat_actors")
@pytest.mark.asyncio
async def test_assess_risk_no_service(
    mock_search_actors,
    mock_search_vulns,
    mock_get_intel,
    mock_threat_intel,
...
async def test_assess_risk_threat_intel_fail(mock_get_intel):
    ...
    mock_get_intel.return_value = ThreatIntelResult(
        indicator="8.8.8.8", error="API Error"
    )
    result = await assess_risk_from_indicator("8.8.8.8")
    assert result.risk_level == "Unknown"
...
@patch("chimera_intel.core.risk_assessment.get_threat_intel_otx")
@patch(
    "chimera_intel.core.risk_assessment.search_vulnerabilities", new_callable=AsyncMock
)
@patch(
    "chimera_intel.core.risk_assessment.search_threat_actors", new_callable=AsyncMock
)
@pytest.mark.asyncio
async def test_assess_risk_probability_levels(
    mock_search_actors, mock_search_vulns, mock_get_intel, pulse_count, expected_prob
):
...
def test_cli_assess_indicator_success(
    mock_asyncio_run, runner, mock_vulnerabilities, mock_threat_actors
):
...
def test_calculate_risk_vuln_with_none_severity(mock_vulnerabilities):
    ...
    vuln_no_sev = Vulnerability(
        cve="CVE-2023-9999", cvss_score=7.5, description="No sev", severity=None
    )
...
@pytest.mark.asyncio
async def test_assess_risk_benign_and_low_impact(
    mock_search_actors, mock_search_vulns, mock_get_intel
):
...
@patch("chimera_intel.core.risk_assessment.asyncio.run")
def test_cli_assess_indicator_no_vulns_with_actors(
    mock_asyncio_run, runner, mock_threat_actors
):
...
@patch("chimera_intel.core.risk_assessment.asyncio.run")
def test_cli_assess_indicator_with_vulns_no_actors(
    mock_asyncio_run, runner, mock_vulnerabilities
):
...
@patch("chimera_intel.core.risk_assessment.asyncio.run")
def test_cli_assess_indicator_no_details(
    mock_asyncio_run, runner, mock_vulnerabilities
):
    """
