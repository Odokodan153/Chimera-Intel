...
def test_analyze_wifi_ssid_decode_error(mock_rdpcap, mock_scapy_packet, capsys):
    ...
    mock_elt_ssid.info.decode.side_effect = UnicodeDecodeError(
        "utf-8", b"", 0, 1, "reason"
    )
...
def test_analyze_wifi_security_types(
    mock_rdpcap, mock_scapy_packet, capsys, crypto_set, expected_security
):
