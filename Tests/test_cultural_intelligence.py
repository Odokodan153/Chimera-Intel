...
def mock_db_no_conn(mocker):
    ...
    mocker.patch(
        "chimera_intel.core.cultural_intelligence.get_db_connection", return_value=None
    )
...
def test_add_cultural_profile_db_connection_fails(
    mock_db_no_conn, mock_console, caplog
):
...
def test_populate_initial_cultural_data(mock_add_profile, mock_console):
    ...
    mock_console.print.assert_called_with(
        "[yellow]Populating initial cultural data...[/yellow]"
    )
