    ...
    return {
        "data_privacy_gdpr": {
            ...
            "rules": [
                {
                    "rule_id": "DP-01",
                    "description": "Data processing must have a legal basis.",
                    "severity": "CRITICAL",
                }
            ],
        },
        "rules_of_engagement_default": {
            ...
            "rules": [
                {
                    "rule_id": "ROE-01",
                    "description": "Offensive operations must not target civilian infrastructure.",
                    "severity": "CRITICAL",
                },
                {
                    "rule_id": "ROE-02",
                    "description": "Operations must have clear and sufficient justification.",
                    "severity": "HIGH",
                },
                {
                    "rule_id": "ROE-03",
                    "description": "A rule with no implementation.",
                    "severity": "LOW",
                },
            ],
        },
...
def test_compliant_operation(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_non_compliant_offensive_operation(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_non_compliant_data_privacy_operation(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_operation_with_insufficient_justification(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
...
def test_audit_no_frameworks_loaded(
    mock_get_frameworks_func,
):  # mock_get_frameworks is injected by decorator
...
def test_audit_rules_module_import_error(
    mock_importlib, mock_get_frameworks_func, mock_frameworks_dict
):
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_audit_missing_rule_function(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
    ...
    assert any(
        "No check function found for rule: ROE-03" in log for log in result.audit_log
    )
    # --- END FIX ---
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_audit_rule_function_exception(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
    ...
    assert any(
        "Rule: ROE-01 failed to execute: Rule engine crashed" in log
...
# ----------------- FIX: Patched the correct target -----------------
@patch("chimera_intel.core.ethint.get_ethical_frameworks")
def test_audit_framework_not_found(
    mock_get_frameworks_func, mock_rules_module, mock_frameworks_dict
):
    ...
    assert any(
        "Framework 'framework_does_not_exist' not found. Skipping." in log
        for log in result.audit_log
    )
    # --- END FIX ---
...
def test_cli_compliant_run(mock_audit, runner, mock_operation_file):
    ...
    mock_audit.return_value = MagicMock(
        is_compliant=True, violations=[], audit_log=["Log entry"]
    )
...
def test_cli_non_compliant_run(mock_audit, runner, mock_operation_file):
    ...
    mock_audit.return_value = MagicMock(
        is_compliant=False, violations=[mock_violation], audit_log=[]
    )
...
def test_cli_invalid_severity(runner, mock_operation_file):
    ...
    result = runner.invoke(
        ethint_app, [mock_operation_file, "--severity-level", "INVALID"]
    )
...
@patch(
    "chimera_intel.core.ethint.audit_operation",
    side_effect=RuntimeError("Frameworks failed to load"),
)
...
@patch("chimera_intel.core.ethint.audit_operation")
def test_cli_severity_filtering_shows_violations(
    mock_audit, runner, mock_operation_file
):
    """Tests that the CLI correctly filters to SHOW violations."""
    v_high = MagicMock(
        framework="F1", rule_id="T-001", severity="HIGH", description="High sev"
    )
    v_low = MagicMock(
        framework="F2", rule_id="T-002", severity="LOW", description="Low sev"
    )
    mock_audit.return_value = MagicMock(
        is_compliant=False, violations=[v_high, v_low], audit_log=[]
    )
...
def test_cli_severity_filtering_hides_all(mock_audit, runner, mock_operation_file):
    ...
    mock_audit.return_value = MagicMock(
        is_compliant=False, violations=[v_low], audit_log=[]
    )
