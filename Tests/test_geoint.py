...
from chimera_intel.core.geoint import (
    ...
    analyze_imagery,  # New import
    geoint_app,
)
from chimera_intel.core.schemas import (
    GeointReport, 
    CountryRiskProfile,
    ImageryAnalysisRequest, # New import
    ImageryAnalysisResult,  # New import
    DetectedObject          # New import
)
...
class TestGeoint(unittest.TestCase):
    ...
    # --- NEW GEOINT++ Function Tests ---

    @patch("chimera_intel.core.geoint.API_KEYS")
    def test_analyze_imagery_mock_success(self, mock_api_keys):
        """Tests the mock imagery analysis function."""
        # Arrange
        mock_api_keys.planet_api_key = "test_key"
        req = ImageryAnalysisRequest(
            target_geofence_id="facility-123",
            center_lat=40.7128,
            center_lon=-74.0060,
            radius_km=1,
        ...
        # Act
        result = analyze_imagery(req, provider="mock")

        # Assert
        self.assertIsInstance(result, ImageryAnalysisResult)
        self.assertEqual(result.status, "COMPLETED")
        self.assertTrue(result.change_detected)
        self.assertIn("New construction", result.change_summary)
        self.assertEqual(result.total_objects, 2)
        self.assertEqual(result.objects_detected[0].label, "Vehicle")
    ...
    def test_cli_run_geoint_analysis_success(
        ...
        # Act
        # FIX: The command name is "run", not the target
    ...
    def test_cli_run_geoint_analysis_no_target(self, mock_resolve):
        ...
        # Act
        # FIX: Call the "run" command without arguments
    ...
    # --- NEW GEOINT++ CLI Tests ---

    @patch("chimera_intel.core.geoint.save_scan_to_db")
    @patch("chimera_intel.core.geoint.resolve_target")
    @patch("chimera_intel.core.geoint.analyze_imagery")
    def test_cli_track_imagery_success(
        self, mock_analyze, mock_resolve, mock_save
    ):
        """Tests the 'geoint track-imagery' CLI command."""
        # Arrange
        mock_resolve.return_value = "facility-123"
        mock_analyze.return_value = ImageryAnalysisResult(
            request_id=str(uuid.uuid4()),
            status="COMPLETED",
            change_detected=True,
            change_summary="Mock change summary",
            objects_detected=[
                DetectedObject(label="Vehicle", confidence=0.9, lat=1.0, lon=1.0)
            ],
            total_objects=1
        )
        ...
        result = runner.invoke(
            ...
            [
                ...
                "--lat", "40.7128", 
                "--lon", "-74.0060",
                "--geofence", "facility-123"
            ]
        )

        # Assert
        self.assertEqual(result.exit_code, 0, msg=result.output)
        mock_resolve.assert_called_with("facility-123")
        mock_analyze.assert_called_once()
        self.assertIn("Mock change summary", result.stdout)
        self.assertIn('"label": "Vehicle"', result.stdout)
        mock_save.assert_called_once_with(
            target="facility-123",
            module="geoint_imagery_analysis",
