...
class TestQLearningAgent(unittest.TestCase):
    ...
    def test_save_and_load_model(self):
        ...
        with patch("builtins.open", mock_file):
            ...
            with patch(
                "json.load", return_value={"(1, 2)": [4, 5, 6]}
            ) as mock_json_load:
...
class TestQLearningLLMAgent(unittest.IsolatedAsyncioTestCase):
    ...
    def test_state_to_tensor(self):
        ...
        )  # sentiment, num_tactics, turn, offer
    ...
    @patch(
        "chimera_intel.core.negotiation_rl_agent.get_cultural_profile", return_value={}
    )
    def test_generate_negotiation_message(self, mock_get_culture):
        ...
        with patch.object(
            self.agent.llm, "generate_message", return_value={"message": "Test OK"}
        ) as mock_gen:
            response = self.agent.generate_negotiation_message(
                state, [], "US", "persona"
            )
            mock_gen.assert_called_once()
            self.mock_ethics.check_message.assert_called_once_with(
                {"message": "Test OK"}
            )
    ...
    def test_generate_negotiation_message_ethics_violation(self):
        ...
        with patch.object(
            self.agent.llm, "generate_message", return_value={"message": "Bad message"}
        ):
