...
def mock_narrative_data():
    ...
    return [
        {
            "content": "There is a great risk of collapse.",
            "source": "Web/News",
            "sentiment": "Negative",
        },
        {
            "content": "This is corrupt and must be stopped.",
            "source": "Blog",
            "sentiment": "Negative",
        },
        {
            "content": "A new solution brings hope.",
            "source": "Web/News",
            "sentiment": "Positive",
        },
    ]
...
def mock_twitter_data_error():
    ...
    return TwitterMonitoringResult(
        query="test", total_tweets_found=0, tweets=[], error="API Error"
    )
...
def test_cwe_init_no_narratives_found(mocker, capsys):
    ...
    mocker.patch(
        "chimera_intel.core.cognitive_warfare_engine.track_narrative", return_value=[]
    )
    mocker.patch(
        "chimera_intel.core.cognitive_warfare_engine.monitor_twitter_stream",
...
def test_engine_generate_narrative_shield(mock_cwe_dependencies, capsys):
    ...
    engine = CognitiveWarfareEngine(
        narrative_query="divisive topic", twitter_keywords=["test"]
    )
    ...
    assert (
        "The strong emotions surrounding 'divisive topic' are understandable."
        in captured.out
    )
...
def test_cli_deploy_shield(mock_engine_class, runner):
    ...
    mock_engine_class.assert_called_with(
        narrative_query="test topic", twitter_keywords=["key1", "key2"]
    )
...
def test_cli_deploy_shield_no_keywords(mock_engine_class, runner):
    ...
    assert result.exit_code == 0
    mock_engine_class.assert_called_with(
        narrative_query="test topic", twitter_keywords=None
    )
    mock_engine_instance.analyze_narratives.assert_called_once()
...
def test_cli_run_scenario_success(mock_scenario_class, mock_run_scenario, runner):
    ...
    # Assert
    assert result.exit_code == 0
    mock_scenario_class.assert_called_with(
        scenario_type="elicitation", target="Asset A"
    )
...
@patch(
    "chimera_intel.core.cognitive_warfare_engine.run_humint_scenario",
