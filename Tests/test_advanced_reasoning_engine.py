...
def mock_gemini_client(mocker):
    ...
    mocker.patch(
        "chimera_intel.core.advanced_reasoning_engine.gemini_client", mock_client
    )
    return mock_client
...
def test_decompose_objective_empty_llm_response_fallback_success(
    ...
    assert (
        "LLM call for objective decomposition returned an empty response" in caplog.text
    )
    assert (
        "Using dynamic fallback, starting with footprint for: evil-corp.com"
        in caplog.text
    )
...
def test_decompose_objective_empty_llm_response_fallback_fail(
    mock_gemini_client, caplog
):
...
def test_generate_reasoning_success(mock_gemini_client, mock_pydantic_data):
    ...
    mock_json_response = {
        ...
        "next_steps": [
            {"module": "vulnerability_scanner", "params": {"host": "1.2.3.4"}}
        ],
...
def test_generate_reasoning_footprint_guard_rail_not_triggered(mock_gemini_client):
    ...
    results = [
        AnalysisResult(module_name="threat_intel", data={"indicator": "example.com"})
    ]
