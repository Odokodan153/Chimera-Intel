...
def get_tech_stack_builtwith(domain: str, api_key: str) -> list:
    ...
    try:
        ...
        # The BuiltWith API has a nested structure. We need to parse it to get technology names.
        technologies = []
        ...
        # Return a list of unique technologies by converting to a set and back to a list
...
def get_tech_stack_wappalyzer(domain: str, api_key: str) -> list:
    """
    Retrieves website technology stack from the Wappalyzer API.
    ...
    try:
        ...
        # Wappalyzer's structure is a bit different, but the goal is the same.
        technologies = []
...
def run_web_analysis(
    ...
    # --- Gather Technology Data from All Sources ---
    ...
    for tech, sources in sorted(all_tech.items()):
        ...
        scored_tech_results.append({
            "technology": tech,
            "sources": sources,
            "confidence": f"{confidence} ({num_found_sources}/{available_tech_sources} sources)"
        })

    tech_stack_report = {"total_unique": len(scored_tech_results), "results": scored_tech_results}

    # --- Gather Traffic Data (single source for now) ---
