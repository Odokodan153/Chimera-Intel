...
async def api_scan(request: Request) -> JSONResponse:
    ...
    try:
        ...
        if scan_type == 'footprint':
            ...
            return JSONResponse(content={"error": "Invalid scan type selected."}, status_code=400)

        if not scan_results:
             return JSONResponse(content={"error": "Scan failed to produce results."}, status_code=500)
        ...
        return JSONResponse(content={"error": "An unexpected server error occurred."}, status_code=500)
...
def run_business_intel_sync_wrapper(company_name: str) -> Dict[str, Any]:
    ...
    intel_data = BusinessIntelData(
        ...
        news=run_business_intel.__globals__['get_news_gnews'](company_name, API_KEYS.gnews_api_key),
